...
export const api = {
  ...
  backupSchedule: () => request<{ enabled: boolean; intervalMinutes: number; destination: string; nextRunAt: string | null }>("/api/snapshots/schedule"),
  setBackupSchedule: (enabled: boolean, intervalMinutes: number, destination: string) => request<{ enabled: boolean; intervalMinutes: number; destination: string; nextRunAt: string | null }>("/api/snapshots/schedule", { method: "POST", body: JSON.stringify({ enabled, intervalMinutes, destination }) }),
