...
import { applyAttachmentRepair, planAttachmentRepair } from "../server/lantern-core.mjs";
...
test("attachment repair creates a review-only proposal and preserves source", async () => {
  ...
  try {
    ...
    const saved = await applyAttachmentRepair(root, { ...proposal, target: "missing.png" });
    assert.equal(saved.path, "Note.md");
    assert.match(await readFile(path.join(root, "Note.md"), "utf8"), /\.\/Assets\/image\.png/);
    await writeFile(path.join(root, "Note.md"), original);
    await assert.rejects(applyAttachmentRepair(root, { ...proposal, target: "missing.png", expectedHash: "stale" }), /changed since diagnostics/);
