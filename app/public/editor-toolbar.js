...
function mountEditorToolbar() {
  ...
  for (const [action, label] of [["bold", "Bold"], ["italic", "Italic"], ["code", "Inline code"], ["link", "Link"], ["heading", "Heading"], ["list", "Bullet list"]]) { const button = document.createElement("button"); button.type = "button"; button.className = "secondary"; button.textContent = label; button.setAttribute("aria-label", label); button.onclick = () => { const result = transformSelection(textarea.value, textarea.selectionStart, textarea.selectionEnd, action); textarea.value = result.value; textarea.setSelectionRange(result.start, result.end); textarea.dispatchEvent(new Event("input", { bubbles: true })); textarea.focus(); }; toolbar.append(button); }
