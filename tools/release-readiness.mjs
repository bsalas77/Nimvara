...
check("SBOM", existsSync(resolve(root, "dist/nimvara.cdx.json")) && existsSync(resolve(root, "implementation/SUPPLY-CHAIN-VALIDATION-2026-08-30.md")) && existsSync(resolve(root, "tools/verify-sbom.mjs")), "CycloneDX SBOM regenerated from locked manifests (480 components) and covered by the committed structural validator; npm audit reports zero vulnerabilities. Signed provenance remains required.");
...
check("RELEASE_ARTIFACT_INTEGRITY", existsSync(resolve(root, "implementation/RELEASE-ARTIFACT-MANIFEST-2026-08-31.json")) && existsSync(resolve(root, "tools/verify-release-artifacts.mjs")) && existsSync(resolve(root, "tools/verify-sbom.mjs")), "A committed artifact manifest and deterministic SHA-256/SBOM verifiers cover the current Windows, Linux, and SBOM files; rerun after every rebuild.");
