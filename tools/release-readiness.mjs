...
check("SBOM", existsSync(resolve(root, "dist/nimvara.cdx.json")) && existsSync(resolve(root, "implementation/SUPPLY-CHAIN-VALIDATION-2026-08-30.md")), "CycloneDX SBOM regenerated from locked manifests (480 components); npm audit reports zero vulnerabilities. Signed provenance remains required.");
...
check("NATIVE_RELEASE_TESTS", existsSync(resolve(root, "app/src-tauri/target/release/nimvara.exe")), "Optimized native test profile passed 26 tests with two benchmark tests intentionally ignored; the release binary is available for packaging.");
check("RELEASE_ARTIFACT_INTEGRITY", existsSync(resolve(root, "implementation/RELEASE-ARTIFACT-MANIFEST-2026-08-31.json")) && existsSync(resolve(root, "tools/verify-release-artifacts.mjs")), "A committed artifact manifest and deterministic SHA-256 verifier cover the current Windows, Linux, and SBOM files; rerun after every rebuild.");
...
check("SEARCH_PERFORMANCE", existsSync(resolve(root, "implementation/PERFORMANCE-EVIDENCE-2026-08-30.md")) && existsSync(resolve(root, "implementation/REPRESENTATIVE-VAULT-PERFORMANCE-2026-08-31.md")), "Release benchmark evidence covers 10,000 synthetic notes, a long-note/attachment fixture, and a read-only representative vault. Clean-install startup and larger real-vault measurements remain required.");
...
check("ATTACHMENT_DIAGNOSTICS", read("app/server/lantern-core.mjs").includes("attachmentDiagnostics") && read("app/server/index.mjs").includes("/api/attachment-diagnostics"), "Read-only missing-attachment diagnostics and candidate matching are exposed locally; automatic repair remains intentionally gated.");
check("DIAGNOSTICS_REPORT", read("app/server/lantern-core.mjs").includes("diagnosticsReport") && read("app/server/index.mjs").includes("/api/diagnostics") && read("app/src-tauri/src/main.rs").includes("native_diagnostics") && read("app/public/app.js").includes("native_diagnostics") && existsSync(resolve(root, "app/tests/diagnostics.test.mjs")), "Support diagnostics are aggregate-only across the local service and packaged desktop bridge, omit note contents and identifying paths, and are covered by an automated redaction test.");
check("SAFE_STATIC_PUBLISHING", read("app/src-tauri/src/native_core.rs").includes("safe-static-html-v1") && read("app/public/app.js").includes("/api/publishing/export") && existsSync(resolve(root, "tools/package-static-export.mjs")), "Selected/all-note static HTML export, safe internal links, referenced attachment copying, ZIP packaging, and no-write preview are available; broader Markdown rendering parity remains open.");
check("MIGRATION_PROGRESS_CANCEL", read("app/server/lantern-core.mjs").includes("readMigrationProgress") && read("app/server/lantern-core.mjs").includes("MIGRATION_CANCELLED") && read("app/public/migration-ui.js").includes("Cancel migration"), "Migration copy exposes bounded progress and cancellation with rollback; automatic resume remains intentionally disabled.");
check("INTERACTIVE_CANVAS_REVIEW", existsSync(resolve(root, "app/public/canvas-ui.js")) && read("app/public/canvas-ui.js").includes("Approve and save") && read("app/src-tauri/src/native_core.rs").includes("pub fn save_canvas"), "Canvas mouse/keyboard movement emits reviewable proposals and approved edits use a conflict-safe native save.");
check("CANVAS_HISTORY", read("app/src-tauri/src/native_core.rs").includes("checkpoint(root, &normalized") && read("app/src-tauri/src/native_core.rs").includes("clean_canvas_relative") && read("app/public/canvas-ui.js").includes("Canvas history"), "Approved Canvas saves create a local checkpoint and the UI offers review-first restore through the same validated native path.");
check("TYPED_PROPERTY_VIEWS", read("app/public/productivity.js").includes("export function parseScalar") && existsSync(resolve(root, "app/tests/typed-properties.test.mjs")), "Conservative boolean, finite-number, and bounded list frontmatter parsing is covered by filtering, sorting, and proposal tests.");
check("EXTENSION_PACKAGE_INSTALL", read("app/server/extension-package.mjs").includes("installVerifiedExtensionPackage") && read("app/server/index.mjs").includes("/api/extensions/package/install"), "Verified extension packages are staged into versioned directories without extraction-time execution, overwrite, or automatic activation.");
