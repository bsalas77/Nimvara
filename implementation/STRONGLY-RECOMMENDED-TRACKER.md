...
| 6 | Extensions | Read-only MCP/API, narrow permission contract, validation, local disabled-by-default registry with enable/disable endpoints, visible local management UI, Ed25519 verification, session-scoped trusted-key registry, refusal to enable untrusted signed extensions, and bounded signed-package verification without extraction | Protected persistent key storage, signed package installation, and adversarial runtime tests |
